package ups.edu.ec.Eva_PaguayLuisDavid_SRV.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        File f = new File("./target/Eva-PaguayLuisDavid-SRV.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "Eva-PaguayLuisDavid-SRV.war").importFrom(f).as(WebArchive.class);
        
        // Add the package containing the test classes:
        war.addPackage("ups.edu.ec.Eva_PaguayLuisDavid_SRV.test");
